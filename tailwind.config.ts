...
const config: Config = {
  ...
  plugins: [
    // Add custom variants and utilities not in Tailwind v3 core
    plugin(function ({ addVariant, addUtilities, matchUtilities, theme }) {
      ...
      // data-checked variant (used by checkbox)
      addVariant("data-checked", "&[data-checked]");

      // data-placeholder variant (used by select)
      addVariant("data-placeholder", "&[data-placeholder]");

      // field-sizing-content utility (CSS property, not in tw v3)
      addUtilities({
